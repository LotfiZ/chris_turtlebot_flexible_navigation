...
@author: Josh Cohen
'''
class TurtlebotFlexPlanner1SM(Behavior):
	'''
	...
	def __init__(self):
		super(TurtlebotFlexPlanner1SM, self).__init__()
		self.name = 'Turtlebot Flex Planner1'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('Receive Path',
										...
										transitions={'planned': 'ExecutePlan YesforDWA/NOforTEB', 'empty': 'Continue', 'failed': 'Continue'},
										autonomy={'planned': Autonomy.Off, 'empty': Autonomy.Low, 'failed': Autonomy.Low},
										remapping={'goal': 'goal', 'plan': 'plan'})

			# x:172 y:335
			OperatableStateMachine.add('ExecutePlan YesforDWA/NOforTEB',
										OperatorDecisionState(outcomes=["yes","no","continue"], hint="Execute the current plan?", suggestion="yes"),
										transitions={'yes': 'Container', 'no': 'TEB', 'continue': 'Continue'},
										autonomy={'yes': Autonomy.High, 'no': Autonomy.Full, 'continue': Autonomy.Full})
