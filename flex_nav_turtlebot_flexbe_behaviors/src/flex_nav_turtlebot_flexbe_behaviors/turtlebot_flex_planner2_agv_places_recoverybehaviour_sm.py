...
from flex_nav_flexbe_states.GetLocationState import GetLocationState
from flex_nav_flexbe_states.CheckLocationState import CheckLocationState
...
@author: Josh Cohen
'''
class TurtlebotFlexPlanner2_AGV_places_RecoveryBehaviourSM(Behavior):
	'''
	...
	def __init__(self):
		super(TurtlebotFlexPlanner2_AGV_places_RecoveryBehaviourSM, self).__init__()
		self.name = 'Turtlebot Flex Planner2_AGV_places_RecoveryBehaviour'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('ExecutePlan',
										...
										transitions={'yes': 'getLocation', 'continue': 'Continue'},
			...
			OperatableStateMachine.add('TEB',
										...
										transitions={'done': 'Log Success', 'failed': 'failed', 'preempted': 'Continue'},
										autonomy={'done': Autonomy.Low, 'failed': Autonomy.High, 'preempted': Autonomy.High},
										remapping={'plan': 'plan'})

			# x:187 y:324
			OperatableStateMachine.add('getLocation',
										GetLocationState(topic="/location", blocking=True, clear=False),
										transitions={'received': 'findLocation', 'unavailable': 'Continue'},
										autonomy={'received': Autonomy.Off, 'unavailable': Autonomy.Off},
										remapping={'plan': 'plan', 'message': 'message', 'Plan': 'Plan'})

			# x:189 y:402
			OperatableStateMachine.add('findLocation',
										CheckLocationState(),
										transitions={'SpecialArea': 'Container', 'NormalArea': 'TEB', 'NotFound': 'TEB'},
										autonomy={'SpecialArea': Autonomy.Off, 'NormalArea': Autonomy.Off, 'NotFound': Autonomy.Full},
										remapping={'message': 'message', 'Plan': 'Plan', 'plan': 'plan'})
