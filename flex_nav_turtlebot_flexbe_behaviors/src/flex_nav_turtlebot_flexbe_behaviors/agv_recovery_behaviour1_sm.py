...
from flex_nav_flexbe_states.Backupcheckstate import Backupcheckstate
...
'''
class AGVRecoveryBehaviour1SM(Behavior):
	'''
	...
	def __init__(self):
		super(AGVRecoveryBehaviour1SM, self).__init__()
		self.name = 'AGV Recovery Behaviour1'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('AGVstop',
										...
										transitions={'done': 'BackdistanceCheck', 'failed': 'EStopFailure'},
			...
			OperatableStateMachine.add('ShortBackup',
										...
										transitions={'done': 'ZeroVelocity', 'failed': 'ClearCostmapQuery'},
			...
			# x:254 y:36
			OperatableStateMachine.add('BackdistanceCheck',
										Backupcheckstate(bumper_topic='/scan'),
										transitions={'long': 'LongBackup', 'short': 'ShortBackup', 'no': 'ClearCostmapQuery'},
										autonomy={'long': Autonomy.High, 'short': Autonomy.Low, 'no': Autonomy.Low})

			# x:676 y:93
			OperatableStateMachine.add('ZeroVelocity',
										agvTimedStopstate(timeout=4, cmd_topic='/cmd_vel', odom_topic='/odom'),
										transitions={'done': 'ClearCostmapQuery', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})
